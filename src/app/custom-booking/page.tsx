...
export default function CustomBookingPage() {
  ...
  const fetchBookings = async (masterAdminId: string) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/masteradmins/${masterAdminId}/custom-bookings`);
      console.log('API status:', res.status);
      if (!res.ok) throw new Error("Failed to fetch bookings");
      const data = await res.json();
      console.log('API data:', data);
      // Map bookingId to id for frontend compatibility
      type BookingApiType = {
        bookingId: number;
        returnDate?: string | null;
        fullName?: string | null;
        customerEmail?: string | null;
        // Add other fields as needed for type safety
      };
      const mapped = data.map((b: BookingApiType) => ({
        ...b,
        id: b.bookingId,
  ...
  const handleEditBooking = async (e: React.FormEvent) => {
    ...
    if (!masterAdminId) return toast.error("No master admin ID");
    try {
      const res = await fetch(`${API_BASE}/api/masteradmins/${masterAdminId}/custom-bookings/${editBookingId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteBooking = async () => {
    ...
    if (!masterAdminId) return toast.error("No master admin ID");
    try {
      const res = await fetch(`${API_BASE}/api/masteradmins/${masterAdminId}/custom-bookings/${deleteId}`, {
        method: "DELETE"
      });
            ...
            <tr key={booking.id} className="border-b border-gray-300">
              ...
              <td className="py-2 px-4 text-center align-middle" style={{minWidth:'100px'}}>
                <div className="flex justify-center items-center gap-3">
                  ...
                  <button className="text-blue-500 hover:text-blue-700 transition-colors text-lg" title="Edit" onClick={() => { setEditBookingId(booking.id); setEditForm((({ id, bookingId, ...rest }) => rest)(booking)); setShowEditModal(true); }}><FaEye /></button>
                </div>
              </td>
            </tr>
          ))}
