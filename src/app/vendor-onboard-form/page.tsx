...
const OnboardingFormComponent = () => {
  ...
  useEffect(() => {
    ...
    if (email) {
      setForm((prev: any) => ({ ...prev, vendorEmail: email }));
    }
  }, [searchParams]);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, field: string) => {
    ...
    if (file) {
      setForm((prev: any) => ({ ...prev, [field]: file }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.update(toastId, { render: err.message || "An error occurred.", type: 'error', isLoading: false, autoClose: 5000 });
    }
  };
    ...
    <div className={`min-h-screen py-12 px-4 transition-colors duration-300 ${isDark ? 'bg-slate-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
      ...
      <div className={`w-full max-w-4xl mx-auto rounded-xl shadow-lg p-8 ${isDark ? 'bg-slate-800' : 'bg-white'}`}>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
          ...
          {/* Bank Details */}
          <div className="md:col-span-2 mt-4 border-t pt-4"><h2 class="text-xl font-semibold">Bank Details</h2></div>
          ...
          {/* ID & Tax Details */}
          <div className="md:col-span-2 mt-4 border-t pt-4"><h2 class="text-xl font-semibold">Identification & Tax</h2></div>
          ...
          {/* File Uploads */}
          <div className="md:col-span-2 mt-4 border-t pt-4"><h2 class="text-xl font-semibold">Document Uploads</h2></div>
